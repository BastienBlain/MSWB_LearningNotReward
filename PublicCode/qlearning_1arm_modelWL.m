function [weight,rpe] = qlearning_1arm_modelWL(choice, reward, alphaP,alphaN,x0)
%
%Inputs:
...
for n = 1:(ntrial),
    ...
    if n<ntrial
        if reward(n)>0
            weight(n+1, 2-choice(n)) = weight(n, 2-choice(n)) + alphaP*rpe(n); % update chosen
        else
            weight(n+1, 2-choice(n)) = weight(n, 2-choice(n)) + alphaN*rpe(n); % update chosen
        end
