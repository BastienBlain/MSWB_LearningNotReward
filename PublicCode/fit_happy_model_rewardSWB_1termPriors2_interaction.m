function result_best = fit_happy_model_rewardSWB_1termPriors2_interaction(term1_mtx,happyscore,priors,constant,type)
...
for istart=1%:nstart
    ...
    if (sum(isnan(priors))>=1)
        if (istart==1)
            ...
            inx = [rangeW1,rangeGamma,rangeCst];%CombVec(rangeW1,rangeGamma',rangeCst)';%[rangeW1,rangeGamma,rangeCst];%CombVec(rangeW1,rangeGamma',rangeCst)';
        else
            inx=inx;
        end
        %inx = [-2 rand rand];
    ...
    lb = [-10 0];
    ub = [10 1]; %max tau of 1.1 or use 1.5
    if exist('constant'),
        if type == 'r'
            inx = [inx];
            lb = [lb 0];
    ...
    result(istart).blabel = {'Term 1','tau','const'};
    result(istart).b  = b;
...
a = x(1); tau1 = x(2); const = x(3);
[happypred] = happy_model_1term_interaction(a,tau1,const, result.term1_mtx);
