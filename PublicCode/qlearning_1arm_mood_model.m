function [weight,rpe] = qlearning_1arm_mood_model(choice, reward, alpha,x0,mood,malpha)
%
%Inputs:
...
alphaall = sig(alpha.*ones(1,80) + malpha.*mood);

for n = 1:(ntrial),
    rpe(n) = reward(n).^x0 - weight(n, 2-choice(n));% compute rpe
    if n<ntrial
        weight(n+1, 2-choice(n)) = weight(n, 2-choice(n)) + alphaall(n)*rpe(n); % update chosen
