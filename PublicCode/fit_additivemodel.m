function [result] = fit_additivemodel(inx,lb,ub, choice, reward, rewardmag, stay)
...
try,
    ...
    result.H           = H; %Hessian
    [loglike, probchoice, weight,rpe,EU,alpha,list_loglike] = model(b, choice, reward, rewardmag); %best fitting model
...
function [loglike, probchoice, weight,rpe,EU,alpha,list_loglike] = model(x, choice, reward, rewardmag)
...
[weight,rpe] = qlearning_1arm_model(choice, reward, alpha,x0); %compute the weights choice, rewardmag, weight,
