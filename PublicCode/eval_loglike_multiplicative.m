function [loglike, probchoice,EU,list_loglike] = eval_loglike_multiplicative(choice, rewardmag, weight, beta,gamma)
...
M1     = rewardmag(:,1);
M2     = rewardmag(:,2);
P1     = max(min(gamma.*(weight(:,1)-0.5)+0.5,1),0);
P2     = max(min(gamma.*(weight(:,2)-0.5)+0.5,1),0);
EU1    = P1.*M1;%w*P1*(1-w)*(M1);
EU2    = P2.*M2;%w*P2*(1-w)*(M2);
EU     = [EU1,EU2];

logodds    = beta * (EU1 - EU2); %compute log odds of choice for each trial
