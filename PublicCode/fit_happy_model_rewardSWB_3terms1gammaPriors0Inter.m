function result_best = fit_happy_model_rewardSWB_3terms1gammaPriors0Inter(term1_mtx,term2_mtx,term3_mtx,happyscore,priors,constant,type)
...
for istart=1%:nstart
    ...
    result(istart).term3_mtx  = term3_mtx;
...
%     if (sum(isnan(priors))>=1) 
        if (istart==1)
            ...
            rangeW2    = 0;%[-0.1 0.1];%[.1.*ones(10,1)];%[-1:1:1];
            rangeW3    = 0;%[-0.1 0.1];%[.1.*ones(10,1)];%[-1:1:1];
            rangeGamma = 0.5;%[0.1:0.1:1]';%;%rand(10,1);%[0.1:0.1:1]';
            rangeCst   = 0.5;%.*ones(10,1);%[0.25:0.25:0.75];
            inx = [rangeW1,rangeW2,rangeW3,rangeGamma,rangeCst];%CombVec(rangeW1,rangeW2,rangeGamma',rangeCst)'; CombVec(rangeW1,rangeW2,rangeGamma',rangeCst)';%
        end
%     else
%         inx = priors;%;
%     end
    ...
    lb = [-3 -3 0 ];
    ub = [ 3  3 1 ]; %max tau of 1.1 or use 1.5
    if exist('constant'),
        if type=='r'
            inx = [inx];
            lb = [lb  0];
...
a = x(1);b = x(2);c = x(3); tau1 = x(4);tau2 = x(4);const = x(end);
[happypred] = happy_model_3terms_inter(a,b,c,tau1,tau2,const,result.term1_mtx,result.term2_mtx,result.term3_mtx);
