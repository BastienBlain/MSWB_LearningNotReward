function [loglike, probchoice,EU,list_loglike] = eval_loglike_additive_mood(choice, rewardmag, weight, beta,gamma,mood,mbeta,mgamma) 
...
gammaall = sig(gamma.*ones(1,80) + mgamma.*mood)';
betaall  = (beta.*ones(1,80) + mbeta.*mood)';

outcome_value_diff = gammaall.*deltaP + (1-gammaall).*deltaM;
logodds            = betaall .* (outcome_value_diff); %compute log odds of choice for each trial
