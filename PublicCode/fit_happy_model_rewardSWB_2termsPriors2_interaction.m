function result_best = fit_happy_model_rewardSWB_1termPriors2(term1_mtx,happyscore,priors,constant,type)
...
for istart=1%:nstart
    ...
    lb = [-2 0];
    ub = [2 1]; %max tau of 1.1 or use 1.5
    if exist('constant'),
        if type == 'r'
            inx = [inx];
            lb = [lb 0];
...
[happypred] = happy_model_1term(a,tau1,const, result.term1_mtx);
