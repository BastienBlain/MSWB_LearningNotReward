function [result] = fit_additivemodel_mood(inx,lb,ub, choice, reward, rewardmag, mood)
...
try,
    inx0 = inx;
    [b, loglike, exitflag, output, lambda, grad, H] = fmincon(@model, inx0, [],[],[],[],lb,ub,[], options, choice, reward, rewardmag,mood);
    ...
    result.H           = H; %Hessian
    [loglike, probchoice, weight,rpe,EU,alpha,list_loglike] = model(b, choice, reward, rewardmag,mood); %best fitting model
...
function [loglike, probchoice, weight,rpe,EU,alpha,list_loglike] = model(x, choice, reward, rewardmag,mood)
...
[weight,rpe] = qlearning_1arm_mood_model(choice, reward, alpha,x0,mood,malpha); %compute the weights choice, rewardmag, weight,
[loglike, probchoice,EU,list_loglike] = eval_loglike_additive_mood(choice, rewardmag, weight, beta, gamma,mood,mbeta,mgamma); %compute the likelihood