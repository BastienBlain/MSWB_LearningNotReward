function result_best = fit_happy_model_rewardSWB_2terms1gammaPriors2Modulator(term1_mtx,term2_mtx,happyscore,priors,constant,type)
...
for istart=1%:nstart
    ...
    lb = [-3 0.1 0 ];
    ub = [ 3  2 1 ]; %max tau of 1.1 or use 1.5
    if exist('constant'),
        if type=='r'
            inx = [inx];
            lb = [lb  0];
...
[happypred] = happy_model_2termsModulator(a,b,tau1,tau2,const,result.term1_mtx,result.term2_mtx);
